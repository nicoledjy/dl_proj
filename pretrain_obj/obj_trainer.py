...
class ObjDetTrainer(Trainer):
    ...
    def __init__(self, model, optimizer, scheduler, trainloader, valloader, device="cpu", scaleX=[100, 70, 50, 20], scaleY=[25, 20, 15, 5]):
        ...
        self.img_w = 306
        self.device = device
        # batch sz is 2 in dataloader
        self.batch_sz = 2
        self.anchor_boxes = self.get_anchor_boxes()
        # load pretrain embedding for the model
    ...
    def train(self, epoch, save=False):
        ...
        for ep in range(epoch):
            ...
            for i, (samples, class_target, box_target) in enumerate(self.trainloader):
                out_pred, out_bbox = self.model(samples.double().to(self.device))
                out_bbox = out_bbox.view(self.batch_sz, -1, 4)
                # out_bbox = out_bbox*10+400
                ...
                loss = self.bbox_loss(box_target.to(self.device), class_target.to(self.device), out_bbox)
                ...
                if i % 10  == 0:
                    ...
                    #print('Epoch: {} | Avg Classification Loss: {} | Memory Allocated: {}'.format(ep, avg_loss, 0))

                torch.cuda.empty_cache()
       

            # avg_loss = float(total_loss / len(self.trainloader))
            print("\nAverage Train Epoch Loss: ", np.mean(train_losses))
            
            #print('Trained Epoch {} | Total Avg Loss: {}'.format(ep, avg_loss))
            validate(ep)
    ...
    def validate(self, epoch, save=False, visualize=False):
        ...
        with torch.no_grad():
            for i, (samples, class_target, box_target) in enumerate(self.trainloader):
                out_pred, out_bbox = self.model(samples.to(self.device))
                out_bbox = out_bbox.view(self.batch_sz, -1, 4)
                out_pred = out_pred.view(self.batch_sz, 9, -1)
                ...
                # vals, inds = torch.max(out_pred, dim=1)
                # main_inds= inds != 0
                # vals = vals[main_inds != 0]
                # inds = inds[main_inds != 0]
                # out_bbox = out_bbox[main_inds]
                # visualizeFast(gt_boxes, out_bbox.double(), vals)
                torch.cuda.empty_cache()
            ...
            # avg_loss = float(total_loss/len(self.valloader))
