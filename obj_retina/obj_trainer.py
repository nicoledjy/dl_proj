...
from anchors import get_bbox_gt, batched_coor_threat_updated_retina, convert_box_format
...
class ObjDetTrainer(Trainer):
    def __init__(self, model, criterion, optimizer, scheduler, trainloader, valloader, device="cpu", scaleX=[100, 70, 50, 20], scaleY=[25, 20, 15, 5]):
    ...
    def encode_input(self, target, sample):
        h = 256
        w = 306

        def box_pre(input_boxes):
            boxes = input_boxes.clone().to(device)
            actual_boxes = []
            for box in boxes:
            ...
            return torch.Tensor((actual_boxes))

        loc_targets = []
        cls_targets = []
        inputs = torch.stack(sample)
        for t, s in zip(target, sample):
            bboxes = t['bounding_box'].to(self.device).clone()
            processed_boxes = box_pre(bboxes)  
            loc_target, cls_target = self.encoder.encode(processed_boxes, t['category'].to(self.device), input_size=(w,h))
    ...
    def train(self, epoch, save=False):
        ...
        for ep in range(epoch):
            ...
            for i, (sample, target, road_image, extra) in enumerate(self.trainloader):
                stacked_samples, loc_targets, cls_targets = self.encode_input(target, sample)
                
                inputs = Variable(stacked_samples.cuda())
                ...
                if i % 10  == 0:
                  print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(ep, i * len(stacked_samples), len(self.trainloader.dataset), 10. * i / len(self.trainloader), loss.item()))
                  self.validate(ep) 
    ...
    def validate(self, epoch, save=False, visualize=False):
        ...
        with torch.no_grad():
            for i, (sample, target, road_image, extra) in enumerate(self.valloader):
                ...
                final_coor, batched_threat_sum = batched_coor_threat_updated_retina(i, self.encoder, loc_preds, target, cls_preds, self.batch_sz, nms_threshold=0.1, plot=False)
                ...
                if i % 20 == 0:
                    print('Val Epoch: {} [{}/{} ({:.0f}%)]\tAverage Loss So Far: {:.6f}'.format(epoch, i * len(stacked_samples), len(self.valloader.dataset), 5. * i / len(self.valloader), np.mean(val_losses)))
        ...
        if save and np.mean(val_losses) < self.best_val_loss:
            ...
            torch.save(self.model.state_dict(), '/scratch/jd4138/bbox_no_pretrain_50.pt')
